...
| EUR benchmark total-returns: Euro Stoxx 600 and Bloomberg Euro Agg Bond index | Bloomberg | `bloomberg_benchmarks.csv` (`Date`, `SXXR_Index`, `LBEATREU_Index`) | `Private_Markets_Simulation.py`, `pm_factor_regression.py`, `summary.py` |
| USD benchmark total-returns: S&P 500, Bloomberg US Agg Bond index | Bloomberg | `LSEG_merged/benchmarks.csv` | `summary.py` |
...
| 3M T-bill | yfinance (`^IRX`), built by `download_benchmarks()` in `Fund_Flows_Regression.py` | `LSEG_merged/benchmarks.csv` | `summary.py` |
